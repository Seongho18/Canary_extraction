...
if __name__ == '__main__':
    ...
    counter_words.update(['<pad>'])


    vocab_words = {w for w, c in counter_words.items() if c >= MINCOUNT}

    with Path(DIR + 'vocab.words.txt').open('w') as f:
    ...
    vocab_chars.update(['<pad>'])


    with Path(DIR + 'vocab.chars.txt').open('w') as f:
    ...
    with Path(DIR + 'vocab.tags.txt').open('w') as f:
